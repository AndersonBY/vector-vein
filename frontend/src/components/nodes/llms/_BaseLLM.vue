...
const props = defineProps({
  ...
  responseFormatAvailable: {
    type: Boolean,
    required: false,
    default: false,
  },
  functionCallAvailable: {
    type: Boolean,
    required: false,
    default: false,
  },
    ...
    <template #main>
      <a-flex vertical gap="small">
        ...
        <BaseField :name="t('components.nodes.llms.common.stream')" name-only type="target"
          ...
          <template #inline>
            <a-checkbox v-model:checked="fieldsData.stream.value">
            </a-checkbox>
          </template>
        </BaseField>

        <BaseFieldsCollapse id="default" :collapse="fieldsData.temperature.group_collpased"
          :name="t('common.more_settings')" @collapseChanged="collapseChanged">

          <BaseField :name="t('components.nodes.llms.common.system_prompt')" type="target"
            v-model:data="fieldsData.system_prompt">
            <a-textarea class="nodrag" v-model:value="fieldsData.system_prompt.value"
              :autoSize="{ minRows: 2, maxRows: 20 }" :showCount="true"
              :placeholder="fieldsData.system_prompt.placeholder" />
          </BaseField>

          <BaseField :name="t('components.nodes.llms.common.temperature')" type="target"
            v-model:data="fieldsData.temperature">
            <TemperatureInput v-model="fieldsData.temperature.value" />
          </BaseField>

          <BaseField name="top_p" type="target" v-model:data="fieldsData.top_p">
            <a-input-number v-model:value="fieldsData.top_p.value" class="nodrag"
              :placeholder="fieldsData.top_p.placeholder" :max="fieldsData.top_p.max ?? null"
              :min="fieldsData.top_p.min ?? null" style="width: 100%;" />
          </BaseField>

          <BaseField v-if="props.responseFormatAvailable" :name="t('components.nodes.llms.common.response_format')"
          ...
          <BaseField v-if="props.functionCallAvailable" :name="t('components.nodes.llms.common.use_function_call')"
            name-only type="target" v-model:data="fieldsData.use_function_call">
            <template #inline>
          ...
          <BaseField v-if="props.functionCallAvailable" v-show="fieldsData.use_function_call.value"
            :name="t('components.nodes.llms.common.functions')" type="target" v-model:data="fieldsData.functions">
            <a-flex vertical gap="small">
